...
#define SET_RC522RST  PCout(4)=1
#define CLR_RC522RST  PCout(4)=0

u8 SPIWriteByte(u8 byte);				//SPI写数据
void SPI1_Init(void);					//SPI初始化
void InitRc522(void);					//初始化RC522
void ClearBitMask(u8 reg,u8 mask);		//清RC522寄存器位
void WriteRawRC(u8 Address,u8 value);	//写RC632寄存器
void SetBitMask(u8 reg,u8 mask);		//置RC522寄存器位
char PcdComMF522(u8 Command, u8 *pIn ,u8 InLenByte,u8 *pOut , u8  *pOutLenBit);//通过RC522和ISO14443卡通讯
void CalulateCRC(u8 *pIn,u8 len,u8 *pOut );		//计算CRC16函数
u8 ReadRawRC(u8 Address);				//读RC632寄存器
...
char PcdWrite(unsigned char addr,unsigned char *pData);	//写数据到卡
char PcdRead(unsigned char addr,unsigned char *pData);	//读卡数据
char PcdHalt(void);						//命令卡片进入休眠状态
void Reset_RC522(void);					//复位RC522
void TurnID(u8 *p);						//显示卡的卡号
unsigned char ReadID(void);						//读取卡号
